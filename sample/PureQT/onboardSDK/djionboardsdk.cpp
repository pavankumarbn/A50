...
void DJIonboardSDK::initFlight() {
  ...
  autoSend = new QTimer();
  autoSend->setInterval(ui->le_flight->text().toInt());  // 20Hz
...
//! @todo test codes
#define link(x) #x
  qDebug() << sizeof(DJI::onboardSDK::MagnetData)
           << link(DJI::onboardSDK::MagnetData);
  startTimer(2);
...
void DJIonboardSDK::closeEvent(QCloseEvent *) {
  ...
  if (!f.open(QIODevice::WriteOnly | QIODevice::Text))
    ...
    f.write(
        QString("ID:").append(ui->lineEdit_ID->text()).append("\r\n").toUtf8());
    f.write(QString("KEY:")
...
void DJIonboardSDK::hotpintReadCallback(CoreAPI *This, Header *header,
                                        UserData userData) {
  DJIonboardSDK *sdk = (DJIonboardSDK *)userData;
...
void DJIonboardSDK::on_cb_flight_autoSend_clicked(bool checked) {
  autoSend->setInterval(ui->le_flight->text().toInt());
  qDebug() << ui->le_flight->text().toInt();
  if (checked)
    autoSend->start();
  else
    autoSend->stop();
...
void DJIonboardSDK::on_btn_coreSet_clicked() {
  ...
  qDebug() << ui->comboBox_1->currentIndex();
...
void DJIonboardSDK::on_tmr_Broadcast() {
  ...
  if (ui->cb_coreTimeStamp->isChecked()) upDateTime();
  if (ui->cb_coreCapacity->isChecked()) upDateCapacity();
  if (ui->cb_coreFlightStatus->isChecked()) upDateFlightStatus();
  if (ui->cb_coreControlDevice->isChecked()) updateControlDevice();
  if (ui->cb_cameraYaw->isChecked()) updateCameraYaw();
  if (ui->cb_cameraRoll->isChecked()) updateCameraRoll();
  if (ui->cb_cameraPitch->isChecked()) updateCameraPitch();
  if (ui->cb_vrcData->isChecked()) updateVirturalRCData();
  if (ui->cb_FlightA->isChecked()) updateFlightAcc();
  if (ui->cb_FlightP->isChecked()) updateFlightPal();
  if (ui->cb_FlightM->isChecked()) updateFlightMagnet();
  if (ui->cb_FlightQ->isChecked()) updateFlightQuaternion();
  if (ui->cb_FlightV->isChecked()) updateFlightVelocity();
  if (ui->cb_FlightPos->isChecked()) updateFlightPosition();
  if (ui->cb_gps->isChecked()) updateGPS();
  if (ui->cb_rtk->isChecked()) updateRTK();
...
void DJIonboardSDK::on_btn_hotPoint_start_clicked() {
  hp->setHotPoint(ui->le_hp_lo->text().toFloat() * DEG2RAD,
                  ui->le_hp_la->text().toFloat() * DEG2RAD,
...
void DJIonboardSDK::wpAddPoint() {
  ...
  waypointData->setItem(number, 1,
                        new QStandardItem(QString::number(
                            flight->getPosition().latitude / DEG2RAD)));
  waypointData->setItem(number, 2,
                        new QStandardItem(QString::number(
                            flight->getPosition().longitude / DEG2RAD)));
...
void DJIonboardSDK::on_waypoint_data_changed(
    const QModelIndex &topLeft __UNUSED,
...
void DJIonboardSDK::on_btn_log_save_clicked() {
  ...
  QString data =
      QFileDialog::getSaveFileName(this, "Save", QString(), "Text (*.txt)");
...
void DJIonboardSDK::on_btn_AbortWaypoint_clicked() { wp->stop(); }

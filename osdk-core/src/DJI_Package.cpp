...
bool DataSubscribe::Package::addByUID(Data::UID uid) {
  ...
  API_LOG(subscribe->getAPI()->getDriver(), ERROR_LOG, "UID %x not found\n",
          uid);
  return false;
}

bool DataSubscribe::Package::addArrayOfUID(
    Data::UID uidArray[], size_t numberOfClause, size8_t id, uint16_t freq,
    bool flagSendTimeStamp,
    DataSubscribe::Package::Callback packageUnpackCallback) {
  if (!uidArray) return false;
  ...
  for (int i = 0; i < numberOfClause; i++) {
    if (!addByUID(uidArray[i])) return false;
  }
